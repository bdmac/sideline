...
**Settings → Manual mode** turns off automatic live rotation selection on this device; it is off by default. Bench drops build or update a saved plan instead of sending players in. Tapping players and **Create plan** provide keyboard-accessible alternatives. In the editor, the **Add swap** person-plus icon below the swaps adds a row with IN / OUT player pickers; the add row disappears when the plan is full. Choose both players or remove an unfinished row before readying the plan. Review the OUT / IN checklist, then **Send players in** when ready. Replacing ordinary pairings by drag updates the plan immediately without confirmation; a drop that conflicts with a linked keeper change opens its editor instead. Warnings and reminders remain, but a deadline does not automatically open the planner.

In either mode, the plan editor shows already-planned incoming players as disabled under **Already going in**, including the current row's checked selection. Edit or remove the reserving swap to free a player for another row. **Plan out** still lets you select a reserved player for **Sub now**, but blocks Add/Update plan for that conflict. **Take out of game** also keeps these players available for immediate replacement.

When a player has no selectable alternatives, their picker becomes a plain name and jersey number, like the review screen. Removing a swap frees its incoming player and restores the other IN pickers; blank rows remain usable while unassigned bench players are available.

In manual planning, taking a field player out requires choosing their bench replacement, unless no bench is available. Switching modes preserves the saved plan, lineup, and timers. Starter selection and filling a short-sided vacancy when a player returns remain unchanged.

Both planners use the bottom **Add swap** icon and individual row removal instead of a **Players to swap** picker. Automatic mode fills each added row with one suggested IN / OUT pairing; manual mode adds a blank row. Existing rows stay unchanged. Remove the final row and choose **Clear plan** to discard a saved plan, or Cancel to keep it. A linked keeper move reserves both field participants, so Add disappears when no valid outgoing slot remains. Adding the final available keeper pair can raise an advisory preference or early-turn warning without reshuffling the earlier swaps.

In either mode, drag a live bench player onto a field player to add or update their planned swap, without sending anyone in. Unrelated pairings stay unchanged; overlapping pairings are replaced without generating extra swaps. With no saved plan, a drop creates just that one pairing. On touchscreens, hold briefly (125ms) before dragging; phones show a temporary pitch so you do not have to scroll while holding a player. Release away from a player to cancel. Tapping a row still opens the keyboard-accessible planner, and Bench / On field tabs use taps rather than swipes.

**Sub now** is the explicit immediate action in both modes. It preserves every unaffected planned pairing and removes only conflicts; it does not regenerate recommendations. To request new recommendations in automatic mode, open **Review → Edit plan** and use the **Refresh suggestions** icon beside Close in the header. This replaces the editor draft with the currently recommended swaps and count. Review and ready it to save, or cancel to retain the previous plan. When the draft already matches, the icon is disabled and labeled **Suggestions are up to date**. It becomes available when your edits or the game situation produce different recommendations. Refresh is also disabled when no further rotations are recommended; existing plans can still be edited and sent.
