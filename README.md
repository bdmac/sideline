...
There is one substitution planner and no Manual mode setting. **Create plan** starts with recommended swaps; a saved plan opens with your existing pairings intact. Edit the IN / OUT choices, review the checklist, then **Send 'em in** when ready. Reaching a rotation deadline automatically opens review or a suggested plan when you are not interacting with another surface; it never sends players automatically. Older Manual mode preferences are ignored without changing your saved game or plan.

The plan editor shows already-planned incoming players as disabled under **Already going in**, including the current row's checked selection. Edit or remove the reserving swap to free a player for another row. **Plan out** still lets you select a reserved player for **Sub now**, but blocks Add/Update plan for that conflict. **Take out of game** also keeps these players available for immediate replacement.

When a player has no selectable alternatives, their picker becomes a plain name and jersey number, like the review screen. Removing a swap frees its incoming player and restores the other IN pickers.

Taking a field player out always requires choosing their bench replacement, unless no bench is available. Nothing is preselected: choose a player, review warnings and planned-swap impacts, then confirm **Remove player**. **Keep player** cancels without changes. With no bench, removal leaves the team short-sided; removing a bench player needs no replacement. Starter selection and filling a short-sided vacancy when a player returns remain unchanged.

The planner uses the bottom **Add swap** icon and individual row removal instead of a **Players to swap** picker. Each added row contains one suggested IN / OUT pairing; existing rows stay unchanged. Remove the final row and choose **Clear plan** to discard a saved plan, or Cancel to keep it. A linked keeper move reserves both field participants, so Add disappears when no valid outgoing slot remains. Adding the final available keeper pair can raise an advisory preference or early-turn warning without reshuffling the earlier swaps.

Drag a live bench player onto a field player to add or update their planned swap, without sending anyone in. Unrelated pairings stay unchanged; overlapping pairings are replaced without generating extra swaps. With no saved plan, a drop creates just that one pairing. On touchscreens, hold briefly (125ms) before dragging; phones show a temporary pitch so you do not have to scroll while holding a player. Release away from a player to cancel. Tapping a row still opens the keyboard-accessible planner, and Bench / On field tabs use taps rather than swipes.

**Sub now** is the explicit immediate action. It preserves every unaffected planned pairing and removes only conflicts; it does not regenerate recommendations. Tap the **Optimize plan** wand beside Close to rebuild the draft for playing-time fairness, which may change pairings and swap count. The header then explains what changed: **Rebuilt with N suggested swaps for fair playing time. Save the plan when you're ready.** There is no extra confirmation; save the result, or Cancel to keep your saved plan. The wand is disabled when recommendations already match or no further rotations are recommended. It becomes available after edits that differ from recommendations. Editing the draft restores the usual planner description.
...
The drag pitch shows each queued incoming player beneath the current player with a substitution icon. For a linked keeper handoff, goal shows the moving outfielder with a Move icon, while their old position shows the bench replacement. This context reflects the saved plan; hovering does not rewrite it.
...
If a linked keeper move is already planned, focused pickers mark its reserved players and explain conflicts beside **Edit keeper plan** after selection. Ordinary replacements in a full plan use **Replace planned swap**, rather than Add to plan. A conflicting bench drop opens the editor without altering the handoff. **Sub now** asks before removing a conflicting handoff; an unrelated immediate swap preserves it without an override dialog.
