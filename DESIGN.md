...
- A compact scoreboard sits directly below the game clock. The persistent Score control replaces Undo in the mobile dock and opens a focused scoring sheet listing only current outfield players; Undo moves into the expanded game log, where it remains available without competing with frequent match actions.
