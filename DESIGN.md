...
- Planned substitutions use shared OUT / IN column headings rather than repeating direction labels in every row. Compact ActionMenu triggers open rich, independently scrolling player menus with name, current stint, total playing time, and position or jersey context. These menus remain anchored to their trigger on larger screens and become consistent bottom-docked selection surfaces on phones, keeping choices within thumb reach while preserving the menu’s selection semantics. OUT options surface the players most due for a break first, while IN options surface the least-played and longest-waiting players first. Already selected players remain visible but semantically disabled in the same column’s other menus, using subdued player metadata and future-tense copy naming the paired player directly (for example, “Planned to come off for Matt”) so normal interaction cannot create duplicate assignments. Disabled rows retain the same selection gutter and right-aligned position/number geometry as available rows without introducing a separate container treatment. Editable rows retain the crossed-arrows substitution symbol grouped with the affected position between the columns. The compact confirmation keeps each row shallow by placing the position code immediately after the arrow and right-aligning incoming names against the IN edge; queued review returns to a vertical arrow/position grouping and a more spacious staging rhythm with jersey-numbered names. A warm, high-visibility live-game banner makes the queued state persistent without obscuring the pitch, and exposes separate Review and Execute actions.
...
Modal bottom sheets lock the underlying document while allowing their content region to scroll, preventing background drift on both iOS and Android. The Plan substitutions dialog uses a fixed header and footer around that scrolling body, is capped at 84% of the viewport so live-game context remains visible, and delegates focus trapping, viewport handling, and nested-menu layering to Primer Dialog. Tapping or clicking the shaded backdrop dismisses every sheet; for destructive confirmations, backdrop dismissal is equivalent to Cancel and never performs the action.
