...
describe("Sideline app", () => {
    ...
    (teamId, periodCount, queued) => {
      ...
      if (queued) {
        ...
        fireEvent.click(
          ...
          ).getByRole("button", { name: "Send 'em in" }),
        );
        expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
      }
      fireEvent.click(
  ...
  it.each([false, true])(
    ...
    (queued) => {
      ...
      if (queued) {
        ...
        fireEvent.click(
          within(planner).getByRole("button", { name: "Save plan" }),
        );
      }
  ...
  it.each([false, true])(
    ...
    (alreadyDue) => {
      ...
      fireEvent.click(
        within(
          screen.getByRole("group", { name: "Bench replacements" }),
  ...
  it("readies substitutions without changing the lineup, then sends them in", () => {
    ...
    const queueButton = screen.getByRole("button", { name: "Save plan" });
  ...
  it("shows the effective substitution after an on-field player becomes unavailable", () => {
    ...
    expect(
      within(confirmation).getByRole("button", { name: "Remove player" }),
    ).toBeDisabled();
    fireEvent.click(
      within(confirmation).getByRole("button", { name: /^Malik #/ }),
    );
    fireEvent.click(
      within(confirmation).getByRole("button", {
        name: "Remove player",
