...
- The game timeline is collapsed by default to protect live-screen space. Expanding it shows the affected player on out-of-game and return events and the OUT / IN players on substitutions.
- Position changes do not count as substitutions. Tapping an occupied player magnet opens that player’s complete action sheet with Plan substitution, Change positions, and Take out of game. Choosing Change positions opens an editor fixed to that player with current-position context and large “Swap with” player/position choice rows matching the substitution pickers; the player cannot be changed inside the editor. Dragging a magnet onto another position moves or swaps the players directly.
...
- The clock automatically pauses exactly at quarter and halftime boundaries, persists the break across refreshes, and presents substitution planning (or ready-plan review) plus an explicit start-next-period action. Regulation time also pauses automatically and prompts the coach to end the game; the coach may continue the clock when needed.
...
- Substitution planning is a persistent two-phase workflow. Readying a plan does not change the lineup or timers; the live screen keeps the ready plan visible and editable until the coach sends every swap in atomically at the actual substitution time. The review screen can remove individual swaps without creating empty replacement rows: removing one reduces the intended plan size by one, and removing the final swap deletes the plan. Stale plans remain visible but cannot be sent in until repaired or cancelled.
- Each available bench player has a direct plan action. It shows that player’s total playing time, current bench stint, and preferred roles, then lets the coach choose one current on-field player to replace; the resulting single swap starts a ready plan or updates that player within the existing ready batch without rebuilding unrelated swaps. Completing this focused action returns directly to the live game so the coach can continue working down the bench; full ready-plan review opens only on explicit request.
